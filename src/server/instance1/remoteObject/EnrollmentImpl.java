...
import server.instance1.logging.MyLogger;
import server.instance1.util.Utils;
import utils.Config;
...
public class EnrollmentImpl implements EnrollmentInterface {
	...
	private server.instance1.util.City City;
	...
	public HashMap<String, Integer> listEventAvailability(String managerId, String eventType) throws RemoteException {
		...
		for (server.instance1.util.City c : City.values()) {
			if (c != this.City) {
	...
	public SimpleEntry<Boolean, String> bookEvent(String customerId, String eventId, String eventType)
		...
		customerSchedule.forEach((ET, events) -> {
			events.forEach((event) -> {
				server.instance1.util.City c = City.valueOf(event.substring(0, 3).toUpperCase());
				if (c == this.City)
		...
		server.instance1.util.City courseCity = City.valueOf(eventId.substring(0, 3).toUpperCase());
	...
	public boolean cancelEvent(String customerId, String eventId, String eventType) throws RemoteException {
		System.out.println("Cancel Event Called");

		server.instance1.util.City eventCity = City.valueOf(eventId.substring(0, 3).toUpperCase());
		boolean result;
	...
	private boolean dropEventOnThisServer(String customerId, String eventId) {
		System.out.println("Drop event on this server called");
	...
	public SimpleEntry<Boolean, String> swapEvent(String customerId, String neweventId, String oldeventId,
		...
		server.instance1.util.City oldeventCity = City.valueOf(oldeventId.substring(0, 3).toUpperCase());
		server.instance1.util.City neweventCity = City.valueOf(neweventId.substring(0, 3).toUpperCase());
	 ...
	 * @return
	 */
	private byte[] udpCommunication(server.instance1.util.City city, Object info, String method) {
		...
		try {
			...
			InetAddress remoteUdpHost = InetAddress.getByName(Config.getStringConfig("INSTANCE" + 1 + "_IP"));
